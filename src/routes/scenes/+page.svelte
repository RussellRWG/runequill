<script lang="ts">
    ...
    onMount(async () => {
        scenes = await invoke<Array<SceneDTO>>("list_scenes");
    });

    function createScene(index: number) {
        invoke("create_scene", { index });
    }
    ...
    {:else}
        <div class="row">
            {#each scenes as scene}
                <!-- Insert point BEFORE scene i -->
                <!-- Block -->
                <SceneCard name={scene.name} text={scene.text}></SceneCard>
            {/each}
            ...
            <div class="insert">
                ...
                <button
                    class="plus small"
                    on:click={() => createScene(scenes.length)}
                    aria-label="Add scene to end">+</button
                >
            </div>
        </div>
    {/if}
